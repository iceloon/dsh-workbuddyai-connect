	...
	*/
	async resolve() {
		...
		if (credential === void 0) {
			...
			throw new Error(`workbuddyai: no signed-in WorkBuddy AI (international) account found; sign in once in the WorkBuddy international desktop app (expected ${desktop} or ${WORKBUDDYAI_AUTH_FILE_ENV})`);
		}
	...
	async refreshNow(credential) {
		if (credential.refreshToken === "") {
			...
			throw new Error("workbuddyai: access token expired and no refresh token is stored; sign in again in the WorkBuddy international desktop app");
		}
		try {
			...
			if (credential.expiresAtMs > Date.now() + 3e4) return credential;
			throw new Error(`workbuddyai: token refresh failed and the access token is expired (${String(error)}); open the WorkBuddy international desktop app once to sign in again`);
		}
	}
...
//#endregion
export { WORKBUDDYAI_DESKTOP_AUTH_BASENAME as A, prepareChatBody as C, randomSentinel as D, probeModel as E, workbuddyAiOwnAuthPath as F, defaultDesktopAuthCandidates as M, defaultDesktopAuthPath as N, WORKBUDDYAI_AUTH_FILENAME as O, parseWorkBuddyAiAuth as P, normalizeCredits as S, PROBE_EFFORT_CANDIDATES as T, parseProductConfig as _, readHostHeartbeat as a, classifyUpstreamError as b, WORKBUDDYAI_CONNECT_VERSION as c, composeCatalog as d, BUILTIN_FREE_MODELS as f, loadProductConfig as g, freeModelIds as h, processStartTimeMs as i, WorkBuddyAiCredentialStore as j, WORKBUDDYAI_AUTH_FILE_ENV as k, FALLBACK_WORKBUDDYAI_MODELS as l, FALLBACK_FREE_MODEL_IDS as m, clearHostHeartbeat as n, workbuddyAiHostHeartbeatPath as o, FALLBACK_EXTRA_MODELS as p, isHeartbeatProcessAlive as r, writeHostHeartbeat as s, WORKBUDDYAI_HOST_HEARTBEAT_FILENAME as t, WorkBuddyAiCatalog as u, workbuddyAiProductConfigPath as v, regionOf as w, isFreeCredits as x, WorkBuddyAiUpstreamClient as y };
