#!/usr/bin/env node
import { F as workbuddyAiOwnAuthPath, a as readHostHeartbeat, c as WORKBUDDYAI_CONNECT_VERSION, g as loadProductConfig, j as WorkBuddyAiCredentialStore, k as WORKBUDDYAI_AUTH_FILE_ENV, l as FALLBACK_WORKBUDDYAI_MODELS, o as workbuddyAiHostHeartbeatPath, r as isHeartbeatProcessAlive, v as workbuddyAiProductConfigPath, y as WorkBuddyAiUpstreamClient } from "./host-heartbeat-BJaBFg8P.js";
...
async function doctor(jsonOutput) {
	...
	const report = {
		...
		desktopAuthFile: {
			path: store.desktopAuthPath() ?? `(no platform default; set WORKBUDDYAI_AUTH_FILE)`,
		...
		productConfig: {
			...
			freeModels: product.source === "cache" ? product.models.filter((model) => model.credits !== void 0 && /^x?0(?:\.0+)?$/u.test(model.credits)).map((model) => model.id) : FALLBACK_WORKBUDDYAI_MODELS.map((model) => model.id)
		},
		hostHeartbeat: {
...
async function status(jsonOutput) {
	...
	if (authStatus.state !== "signed-in") {
		...
		else process.stdout.write(`WorkBuddy AI Connect: signed out\nHost bundle: ${hostState}\n`);
		return 1;
	}
	let credits;
	try {
	...
	process.stdout.write([
		`WorkBuddy AI Connect: signed in${authStatus.nickname === void 0 ? "" : ` as ${authStatus.nickname}`}`,
