...
import { prepareChatBody, WorkBuddyAiUpstreamClient, type UpstreamErrorKind } from './upstream.ts'
...
export interface WorkBuddyAiShim {
...
export interface WorkBuddyAiShimOptions {
 ...
 */
export function createWorkBuddyAiShim(options: WorkBuddyAiShimOptions): WorkBuddyAiShim {
  const { store, client, catalog } = options
  const logger = options.logger

  // Per-process shared secret. Lives only in memory; the adapter resolves it as
  // the OpenAI apiKey, which pi-ai sends as `Authorization: Bearer ...`. The
  // shim never forwards it upstream — the real credential comes from the store.
  // A local attacker who can hit the port still cannot forge this.
  ...
  const baseUrl = (): string => {
    ...
    if (address === null || typeof address === 'string') {
      throw new Error('workbuddyai shim has no listening address')
    }
  ...
  async function handle(req: IncomingMessage, res: ServerResponse): Promise<void> {
    try {
      ...
      // always satisfies both; DNS-rebinding pages and cross-origin POSTs do not.
  ...
  async function chatCompletions(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    if (!result.ok) {
      writeOpenAIError(
        ...
        result.kind,
        `workbuddyai upstream ${result.kind} (http ${result.status}): ${result.message.slice(0, 400)}`,
      )
      return
    }
