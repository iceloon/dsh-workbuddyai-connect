...
export const WORKBUDDYAI_HOST_HEARTBEAT_FILENAME = '.workbuddyai-host-heartbeat.json'
...
export interface WorkBuddyAiHostHeartbeat {
...
export function workbuddyAiHostHeartbeatPath(): string {
...
export async function writeHostHeartbeat(): Promise<void> {
  const document: WorkBuddyAiHostHeartbeat = {
...
export async function clearHostHeartbeat(): Promise<void> {
  try {
    ...
    // Best-effort cleanup; a stale heartbeat is harmless (the reader detects a PID mismatch).
  }
}

/** Read and validate the heartbeat; returns `undefined` when absent or malformed. */
export async function readHostHeartbeat(): Promise<WorkBuddyAiHostHeartbeat | undefined> {
  let raw: string
  try {
  ...
  try {
    const parsed = JSON.parse(raw) as Partial<WorkBuddyAiHostHeartbeat>
    if (
      parsed.version === HEARTBEAT_FORMAT_VERSION
      && parsed.package === HEARTBEAT_PACKAGE
 ...
 */
export function isHeartbeatProcessAlive(heartbeat: WorkBuddyAiHostHeartbeat): boolean {
  try {
